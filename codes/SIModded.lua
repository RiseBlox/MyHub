...
do
	...
	local function FetchAPI()
		...
		if not ok or not API_Dump then
			-- ? Version-specific dump unavailable (common on some executors / CDN); silently fall back to the latest dump below.
			-- ? A genuine total failure is still reported by "Failed to load the API Dump" further down.
...
local function RunDecompilePrepass(ldeccache, O, report)
	local HttpRequest = request or http_request
	if not (getscriptbytecode and HttpRequest and sha384 and base64encode) then
		return false, "needs getscriptbytecode + request/http_request"
	end
	...
	local function consider(o)
		...
		if ok and type(bc) == "string" and bc ~= "" then
			local key = sha384(bc)
			if not seen[key] and ldeccache[key] == nil then
				seen[key] = true
				jobs[#jobs + 1] = { key = key, bc = bc }
			end
		end
	end
		...
		task.spawn(function()
			pcall(function() -- guard: a throw here must NOT skip `active -= 1`, or the wait below hangs forever
				while true do
					...
					local body = decompile_via_api(job.bc, apiUrl, rateGap)
					if body and body ~= "" then
						-- strip null bytes like the normal decompile path does (they break the XML file)
...
local function synsaveinstance(CustomOptions, CustomOptions2)
	...
	local used_lua_expert = false -- set true when lua.expert (external API) is the active decompiler; credits it in script headers
							...
							elseif "ProtectedString" == ValueType then -- TODO: Try fitting this inside Descriptors
								...
								if PropertyName == "Source" then
									if DecompileIgnoring then -- ? Should this really prevent extraction of the original source if present ?
										...
										value = ""
											.. (used_lua_expert and "-- DECOMPILED USING LUA.EXPERT FREE DECOMPILER — Join: https://discord.com/invite/y63m4zUYa4\n" or "")
											.. "\n"
			...
			local streamOk, streamReason = StreamEntireMap(OPTIONS, function(streamed, totalChunks, eta)
				if StatusText then
					StatusText.Text = string.format(
						...
						streamed,
						totalChunks,
						eta and string.format(" (~%ds left)", math.ceil(eta)) or ""
					)
				end
